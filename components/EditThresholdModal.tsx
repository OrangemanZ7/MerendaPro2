...
const thresholdSchema = z.object({
  ...
  quantityPerPackage: z
    .number()
    .min(1, "A quantidade por embalagem deve ser pelo menos 1")
    .optional(),
});
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        quantityPerPackage: product.quantityPerPackage || 1,
      });
    }
  }, [product, reset]);
      ...
      const res = await fetch(`/api/products/${product._id}`, {
        ...
        body: JSON.stringify({
          lowInventoryThreshold:
            data.lowInventoryThreshold !== undefined &&
            !isNaN(data.lowInventoryThreshold)
          ...
          quantityPerPackage: data.quantityPerPackage || undefined,
        }),
      });
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {product.category === "meal" && (
                  <p className="mt-1 text-xs text-amber-600">
                    O preço de produtos de alimentação é definido
                    automaticamente pelo contrato ativo.
                  </p>
                )}
                {errors.price && (
              ...
              <div>
                ...
                <input
                  type="number"
                  {...register("lowInventoryThreshold", {
                    valueAsNumber: true,
                  })}
                  className="w-full rounded-md border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Se deixado em branco, o sistema usará o limite global de{" "}
                  {globalThreshold}.
                </p>
              </div>
            </div>
