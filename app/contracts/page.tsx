      ...
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Gerencie contratos públicos, valores e limites de compra para
            ingredientes de alimentação.
          </p>
        </div>
        <Link
          href="/contracts/new"
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-slate-500">
            ...
            <p className="mt-1">
              Comece criando um novo contrato de alimentação.
            </p>
            <Link
              href="/contracts/new"
                ...
                {contracts.map((contract) => {
                  const totalValue =
                    contract.items?.reduce(
                      (sum: number, item: any) =>
                        sum + item.maxQuantity * item.pricePerUnit,
                      0,
                    ) || 0;
                  const purchasedValue =
                    contract.items?.reduce(
                      (sum: number, item: any) =>
                        sum + (item.purchasedQuantity || 0) * item.pricePerUnit,
                      0,
                    ) || 0;
