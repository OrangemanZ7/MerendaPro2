  ...
  const handlePrint = () => {
    ...
    if (reportType === "inventory") {
      ...
      rows = data.map((item) => {
        ...
        const supplierName =
          item.contract?.supplier?.name || item.supplierName || "N/A";
        return [
          new Date(item.createdAt).toLocaleDateString("pt-BR"),
          supplierName,
          item.status,
    ...
    if (selectedYear) {
      doc.text(
        `Período: ${selectedMonth ? `${parseInt(selectedMonth) + 1}/` : ""}${selectedYear}`,
        titleX,
        titleY + 16,
      );
    }
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Gere relatórios e visualize dados do sistema.
          </p>
        </div>
