...
export async function GET(request: Request) {
  try {
    ...
    const processedConsumptions = consumptions.map((item: any) => {
      if (
        item.product &&
    ...
    return NextResponse.json(processedConsumptions);
  } catch (error: any) {
...
export async function POST(request: Request) {
  try {
    ...
    if (body.items && Array.isArray(body.items)) {
      ...
      const alias =
        locationDoc.alias || locationDoc.name.substring(0, 2).toUpperCase();
      ...
      for (const item of items) {
        ...
        if (!inventoryItem || inventoryItem.quantity < item.quantity) {
          return NextResponse.json(
            { error: `Estoque insuficiente para um dos produtos selecionados` },
            { status: 400 },
          );
        }
      }
    ...
    if (!inventoryItem || inventoryItem.quantity < quantity) {
      return NextResponse.json(
        { error: "Estoque insuficiente para este consumo" },
        { status: 400 },
      );
    }
