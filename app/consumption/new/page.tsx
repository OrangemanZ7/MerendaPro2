...
export default function NewConsumptionPage() {
      ...
      const fetchInventory = async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setInventory(
              data.filter((item: any) => item.quantity > 0 && item.product),
            );
          }
        } catch (err) {
  ...
  const onSubmit = async (data: ConsumptionFormValues) => {
    ...
    if (!finalLocation || !user?.id) {
      setError(
        "Informações de usuário ou local não encontradas. Por favor, selecione um local.",
      );
      return;
    }
    ...
    for (const item of data.items) {
      const inventoryItem = inventory.find(
        (inv) => inv.product._id === item.product,
      );
      if (inventoryItem && item.quantity > inventoryItem.quantity) {
        setError(
          `Quantidade excede o estoque disponível para o produto ${inventoryItem.product.name} (${inventoryItem.quantity} disponíveis)`,
        );
        return;
      }
    }
      ...
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Dê baixa em produtos do seu estoque.
          </p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {selectedInventoryItem && (
                            <span className="text-sm text-slate-500 whitespace-nowrap">
                              / Máx: {selectedInventoryItem.quantity}{" "}
                              {selectedInventoryItem.product.unitType
                                ?.abbreviation ||
                                selectedInventoryItem.product.unit}
                            </span>
                          )}
                        </div>
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            type="submit"
            disabled={
              isSubmitting || isLoadingInventory || inventory.length === 0
            }
