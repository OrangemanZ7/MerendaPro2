...
export default function ConsumptionPage() {
  ...
  const isRestrictedUser =
    user?.role === "dependency" || user?.role === "warehouse";
  ...
  const groupedConsumptions = consumptions.reduce(
    (acc, current) => {
      // Group by batchId if available, otherwise fallback to grouping by exact createdAt timestamp + location + user
      const groupId =
        current.batchId ||
        `${current.createdAt}_${current.location?._id}_${current.consumedBy?._id}`;
  ...
  const sortedGroups = Object.values(groupedConsumptions).sort(
    (a: any, b: any) =>
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Tem certeza que deseja excluir este registro de consumo? A quantidade será devolvida ao estoque.",
      )
    ) {
      return;
    }
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Registre e acompanhe o consumo de produtos no local.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {sortedGroups.map((group: any) => (
                  <React.Fragment key={group.id}>
                    <tr
                      className={`hover:bg-slate-50 transition-colors ${expandedGroups[group.id] ? "bg-slate-50" : ""}`}
                    >
                      <td className="px-6 py-4 whitespace-nowrap font-mono text-emerald-700 font-medium">
                        {group.code ||
                          `CS-${new Date(group.createdAt).toISOString().split("T")[0]}-${group.location?.alias || group.location?.name?.substring(0, 2).toUpperCase() || "XX"}`}
                      </td>
