...
export default function NewOrderPage() {
  ...
  const availableProducts =
    selectedType === "contract" && selectedContract
  ...
  const handleProductChange = (index: number, productId: string) => {
    if (selectedType === "contract" && selectedContract) {
      const contractItem = availableProducts.find(
      ...
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Crie um pedido para reabastecer o Armazém Central.
          </p>
        </div>
      </header>
                ...
                >
                  ...
                  {contracts.map((c) => (
                    <option key={c._id} value={c._id}>
                      {c.contractNumber} -{" "}
                      {c.supplier?.alias || "Fornecedor Desconhecido"}
                    </option>
                  ))}
                </select>
                {errors.contract && (
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              type="button"
              onClick={() =>
                append({ product: "", quantity: 1, pricePerUnit: 0 })
              }
                      ...
                      >
                        ...
                        {availableProducts.map((p: any) => (
                          <option key={p._id} value={p._id}>
                            ...
                            {p.packageType
                              ? `1 ${p.packageType} de ${p.quantityPerPackage} ${p.unitType?.abbreviation || p.unit}`
                              : p.unitType?.abbreviation || p.unit}
                          </option>
                        ))}
                      </select>
